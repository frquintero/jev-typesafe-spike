...
> Versión 6 — 23 de septiembre de 2026
...
**Sobre el vocabulario.** Esta guía se ha construido a lo largo de varios spikes, y su vocabulario ha cambiado con lo aprendido. Desde la versión 6 usa el **marco del juez** (§1): el `state` es el **expediente**, el campo `instructions` contiene el **juicio** y el resultado es un **grado de soporte**. Donde el API dice *question* o *answer*, la guía dice juicio y resultado.
...
expediente (state) + juicio (instructions [+ criteria])
                        ...
                        Jev
                         ↓
      grado de soporte (y su reparto entre completaciones)
                         ↓
                       código
```
...
**Las tres formas del juicio.**

| Primitiva | Forma del juicio | Ejemplo | Qué devuelve Jev |
|---|---|---|---|
| Noul | **cerrado**: un juicio completo | «`ticket` pide un reembolso.» | el grado en que sostiene ese juicio |
| Choice | **abierto**: un juicio con un hueco; `criteria` trae las formas de completarlo | «`ticket` corresponde al área ___» → facturación · técnica · cuenta · otra | cómo reparte su soporte entre las completaciones |
| Score | **abierto sobre una escala**: `criteria` trae niveles ordenados | «La urgencia de `mensaje` es ___» → sin plazo · esta semana · hoy · ya | el reparto entre niveles y su posición media |

El juicio abierto es lo que la lógica llama una *función proposicional* (Frege, Russell): una forma de juicio con un lugar vacío, que se vuelve juicio completo al llenarlo.
...
1. ¿Lo que necesitas se puede escribir como un juicio cerrado, o como un juicio abierto con completaciones o niveles definidos?
...
Un **Noul** somete a Jev un **juicio cerrado** y devuelve `noul`: el grado en que Jev sostendría ese juicio con el expediente a la vista. La documentación lo describe como la probabilidad de que el enunciado sea verdadero.
...
Una **Choice** somete a Jev un **juicio abierto**: un juicio con un hueco. Los `criteria` son las formas de completarlo, y Jev **reparte su soporte** entre esas completaciones.
...
- `probabilities`: cómo reparte Jev su soporte entre las completaciones. Suma 1.
- `choice`: la completación que concentra más soporte.
- `confidence`: qué tan concentrado está el reparto (§5).

**Jev no responde: sopesa completaciones.** Si le das «¿Qué hora es?» con las opciones 10:00, 11:00 y 1:00, sin expediente, devuelve igual un reparto entre las tres. No sabe la hora: sopesa tres juicios posibles («son las 10:00», «son las 11:00», «es la 1:00»). En la práctica el campo se suele escribir con signo de interrogación, pero lo que contiene es un juicio abierto.

La lista de completaciones debe ser explícita y puede tener como máximo 255 opciones. **Incluye una opción de salida** —`otra`, `ninguna`, `no se indica`—, porque puede que ninguna completación se sostenga con el expediente; sin ella, el soporte se reparte a la fuerza entre las que hay. Si las completaciones se solapan, están incompletas o cambian con frecuencia, el resultado exige evaluación adicional.

### 3.3 Score

Un **Score** somete a Jev un **juicio abierto sobre una escala ordenada** que tú defines: «la urgencia de este mensaje es ___». Los niveles son las completaciones, en orden.
...
| 5 | **Sin dobles negaciones ni indirección.** | Jev lee al pie de la letra las negaciones y las palabras de alcance, y sopesa con menos fiabilidad las dobles negaciones y la indirección (límites 1 y 4 de la sección 6). | Un nivel con tres negaciones («solo… nadie más… nada») bajó la confianza de 0,98 a 0,77. Ese nivel tenía además un error conceptual (esperar no es una consecuencia de la demora), así que el caso ilustra la regla, no la prueba. |
...
## 4. El expediente, el juicio y sus completaciones

En el API, un request tiene tres piezas: `state`, `instructions` y, en Choice y Score, `criteria`. En el marco del juez (§1) son el **expediente**, el **juicio** y sus **completaciones**.
...
### 4.2 Las completaciones (`criteria`)

En una Choice o un Score, `criteria` trae las **completaciones** del juicio abierto: qué significa cada opción o cada nivel. No deberían reducirse a etiquetas vagas. La documentación de Choice permite describir cada completación con un objeto de campos `what`, `not_for` y `examples`:
...
La estructura hace el juicio más inspeccionable, pero no garantiza que las completaciones sean mutuamente excluyentes ni que los ejemplos cubran producción. En una Noul, `criteria` es opcional: con los campos `true` y `false` describe qué significa sostener o no el juicio.

### 4.3 El juicio (`instructions`)

El campo se llama `instructions` en el API, y ese nombre no se puede cambiar en el request. Pero **no contiene instrucciones: contiene el juicio** que Jev va a sopesar —cerrado en la Noul, abierto en Choice y Score (§1)—. Tampoco es una pregunta, porque Jev no responde nada. En esta guía lo llamamos **juicio**.
...
- **Evita dobles negaciones e indirección compleja**: Jev las sopesa con menos fiabilidad (límite 4 de la sección 6).
- **Escribe exactamente lo que quieres decir.** Jev lee al pie de la letra las palabras de alcance («alguno», «todos», «solo»), las negaciones y las condiciones implícitas (límite 1). Una palabra de alcance bien elegida evita términos medios: «¿Tiene **alguna** experiencia en Python?».
- **Que el juicio y las completaciones no se contradigan** (límite 7).
...
{
  "instructions": {
    "question": "La función que cumple este fragmento dentro del documento es ___",
    "focus": "La función dentro del documento, no la calidad literaria"
  }
}
```

La documentación usa en sus ejemplos campos como `question`, `focus`, `compare` e `inspect`. Pese al nombre `question`, lo que se escribe ahí es el juicio. Puedes añadir campos propios, pero sus nombres son diseño de tu aplicación, no parte de un esquema oficial.
...
- Elige la primitiva por la forma del juicio: cerrado → Noul; abierto con completaciones sin orden → Choice; abierto con niveles ordenados → Score.
- Escribe siempre qué se juzga. Si el objeto queda implícito, Noul y Choice pueden divergir; si queda explícito, coinciden.
- En una Choice, trata el menú como parte del juicio: cada completación con su descripción y, por regla general, una opción de salida.
...
**Una hipótesis (nivel 4): la incoherencia interna del expediente.** En los casos que más bajan, el resto de la nota describe al protagonista notorio, no al nombrado: los perros eran de Amundsen (Scott usó ponis, trineos a motor y arrastre a pie), y la transmisión televisada fue la del paseo lunar de Armstrong. Algo parecido, más débil, ocurre con Toronto («bilingüe» describe a Ottawa), con el morado («para pintar las hojas» pide verde) y con Sídney («ordenada» describe a Canberra, una ciudad planificada). En la ballena y en «2+2=5» no hay detalles que delaten nada, y el grado casi no baja. Lope (0,76) baja sin detalles que delaten: lo notorio pesa por sí solo, pero menos. Si la hipótesis se confirma, esos expedientes no solo chocan con lo notorio: **se contradicen a sí mismos**, y ante un expediente así un juez prudente no se pronuncia sobre lo que «afirma». Se prueba cambiando solo el detalle que delata (por ejemplo, la nota de Scott con un detalle neutro, o con uno coherente con Scott).
...
- Si un caso cae en la franja central, revisa primero el juicio y luego la coherencia del expediente (§3.1).

Crudos: `falso-c{1..6}-*`, `diag-scott-*`, `diag-2mas2-*`.
...
**Qué dice la documentación (nivel 1).** `confidence` es un estadístico calculado a partir de la distribución de probabilidades: si el soporte se concentra en una completación, el reparto es nítido; si se reparte, no lo es. Como ejemplo da la fórmula para tres opciones, `(3 × probabilidad máxima − 1) / 2`, y aclara que es una medida práctica, no una definición cerrada: si otra medida te sirve mejor, calcúlala tú desde `probabilities`.
...
- **las completaciones se pisan**: el caso cabe en dos opciones o niveles;
- **el expediente no decide ese caso**: el propio texto admite dos lecturas.

En el Anexo A, la confianza baja señaló los tres casos problemáticos (0,37; 0,25; alrededor de 0,70). Cuando aparece, mira `probabilities` para ver entre qué completaciones se reparte el soporte, y actúa sobre el juicio, las completaciones o el flujo.
...
4. **Indirección**: los juicios con dobles negaciones o indirección compleja se sopesan con menos fiabilidad.
...
7. **Juicio y criterios contradictorios**: si `instructions` y `criteria` chocan, o las completaciones son ambiguas o incompatibles, Jev puede confundirse.
8. **Invariantes entre juicios**: los resultados de juicios independientes no tienen por qué ser coherentes globalmente.
...
LLM u operador
  └─ define o revisa la tarea abierta
       ↓
juicios y completaciones versionados
       ↓
...
5. prueba de sensibilidad a la redacción del juicio, a las completaciones y a su orden;
...
| Juicio | Lo que Jev sopesa frente al expediente. Va en el campo `instructions` del API. |
| Juicio cerrado | Juicio completo; es la forma de la Noul. |
| Juicio abierto | Juicio con un hueco (una función proposicional); es la forma de Choice y Score. |
| Completación | Cada forma de llenar el hueco de un juicio abierto; van en `criteria`. |
...
| Noul | Juicio cerrado; su valor es el grado de soporte de ese juicio. |
| Choice | Juicio abierto con completaciones sin orden; Jev reparte su soporte entre ellas. |
| Score | Juicio abierto sobre niveles ordenados; su `score` es la posición media del reparto, no un nivel elegido. |
...
| Probabilities | Reparto del soporte entre completaciones o niveles. |
| Confidence | Qué tan concentrado está el reparto; útil para enrutar, no garantía individual de acierto. |
| Criteria | Las completaciones: qué significa cada opción o nivel, con sus límites y ejemplos. |
| Instructions | Nombre del campo en el API; contiene el juicio. |
...
Diseñar con Jev es, sobre todo, **redactar bien dos cosas**: el juicio exacto que se quiere sopesar y el expediente frente al que se sopesa. Cuando el grado no es el esperado, lo primero es preguntarse si el juicio es el que se quería y si el expediente es coherente.

La frontera sana es:

> **Jev sopesa; el sistema decide.**

Cuando el juicio es estrecho, el expediente está disponible, las completaciones son explícitas y existe evaluación, Jev merece una prueba. Cuando la tarea exige investigación, deliberación prolongada, aritmética, invariantes globales o autoridad sobre efectos irreversibles, hay que dividirla y dejar esas responsabilidades fuera del modelo.
...
| El juicio y las completaciones no deben contradecirse. | P | diseño | oficial | §4.3, §6 |
...
| Los IDs de los juicios no llegan al modelo. | Pr | modelo | oficial | §6 |
...
| Con juicios bien redactados, las réplicas casi no varían. | Pr | modelo | medida | Anexo B, P1–P3 |
...
| Baja más cuanto más se contradice el propio expediente (nombra a uno y describe a otro). | Pr | modelo | conjetura | §4.6 |
...
| Regla | Clase | Ámbito | Estado | Dónde |
|---|---|---|---|---|
| Una Noul somete un juicio cerrado y devuelve su grado de soporte; no reparte entre «sí» y «no». | P | modelo | oficial + aceptada | §3.1 |
...
| Redacta la Noul como un juicio cerrado (un enunciado), no como una pregunta. | P | diseño | aceptada | §3.1, §4.3 |
...
| Regla | Clase | Ámbito | Estado | Dónde |
|---|---|---|---|---|
| Una Choice somete un juicio abierto; Jev reparte su soporte entre las completaciones. No responde. | P | modelo | aceptada | §3.2 |
| Completaciones explícitas, cada una con su descripción. | P | diseño | oficial | §3.2, §4.2 |
| Incluye una opción de salida: puede que ninguna completación se sostenga. Sin ella, lo que no encaja cae igual en la mejor completación y la confianza no lo detecta (medido con classifier.dev, mismo modelo). Cuando alguna encaja, la salida no roba soporte (≤ 0,01 en la prueba 2). | P | diseño | oficial + medida | §3.2, Anexo B, P2 |
| Para completaciones cercanas, usa `what` / `not_for` / `examples`. | P | diseño | oficial | §4.2 |
| El menú es parte del juicio: cambiar las completaciones puede cambiar el reparto. | Pr | modelo | medida (un caso) | §4.5 |

### Score

| Regla | Clase | Ámbito | Estado | Dónde |
|---|---|---|---|---|
| Un Score somete un juicio abierto sobre niveles ordenados; el score es la posición media del reparto, no una medida. | P | modelo | oficial | §3.3 |
...
| La confianza baja (y la franja central de la Noul) es una señal de diseño: juicio mal redactado, completaciones que se pisan o un caso que el expediente no decide. | P | diseño | medida | §3.1, §5 |
...
- **Incoherencia interna del expediente**: la nota de Scott con un detalle neutro y con uno coherente con Scott (§4.6).
- **Juicio abierto sin signo de interrogación**: si escribir «La capital, según `folleto`, es:» en lugar de «¿Cuál es la capital…?» cambia el reparto.

**Resueltas:** «¿qué pasa en una Choice sin opción de salida?» (cubierta por la documentación y por lo medido con classifier.dev; no requiere prueba propia); «¿aseveración o presuposición?» (el 0,82 del folleto de Sídney): no era la presuposición, sino el choque con lo notorio (Anexo B, P3a).

**Retiradas:** «un state impreciso contamina» (§4.4), «la primitiva elige el mundo de referencia» (§4.5), la fecha de corte «hacia el 2T de 2025» (§6), la lectura de la franja central como «no sé» o como «duda» de Jev (§3.1).
