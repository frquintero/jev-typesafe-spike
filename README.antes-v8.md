...
## Marco conceptual vigente (guía v6)
...
(0,30–0,65) es una señal de diseño. Detalle en
`jev_typesafe_guia_pedagogica_v2.md` §1 y §3.1, y en `diccionario.md`.
...
4. Los IDs de los juicios viajan pero el modelo no los usa (doc
   literal); sirven para casar cada resultado en código.
...
14. La redacción del juicio y el idioma se testean con datos (gold +
    ...
    juicios de `criteria` (TypeSafe, límites 1, 4 y 7). Vale igual
...
21. La confianza (Choice/Score) y la franja central (Noul) se usan para
    ...
    cruzaba el 0,7 entre réplicas). Ver guía §5.
...
- Pruebas de reglas (2026-09-22/23): scripts en `probes/`
  ...
  `instructions_vacio.py`); lectura y
