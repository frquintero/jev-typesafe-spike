...
- Valor: el nombre o expresión de una posición o elemento admitido dentro de una escala, utilizado para expresar una determinación bajo una variable. Puede ser una cantidad (21,4 °C, 70 kg), una categoría (rojo, casado), un nivel ordenado (bajo, medio, alto) o una fecha (25 de septiembre de 2026). Si no es ninguna de estas, no es un valor.
- Condiciones constitutivas: lo que, además del caso y la variable, fija la pregunta; si cambiara, la pregunta sería otra. No es de dónde proviene el valor ni quién lo dice.
...
3. Solo hay dato si puede formularse la pregunta <variable>(<caso>, <condiciones constitutivas>) = ? y `texto` da un valor que la cierra. Si falta el caso, la variable, el valor o su escala, no hay dato. Dejar fuera lo que no es dato es parte de la tarea.
4. Registra lo que es, fue o será, no lo que debería ser.
5. Quien dice, sostiene, atribuye, planea o mide algo no forma parte del dato: es de donde proviene. Examina lo dicho como cualquier otra parte de `texto`.
